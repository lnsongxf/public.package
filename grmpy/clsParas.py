...
class ParasCls(MetaCls):
    ...
    def get_parameters(self, type_, subgroup, is_obj=False):
        ...
        if type_ == 'choice':
            ...
            coeffs_bene = np.concatenate((coeffs_bene_ex_ante, np.tile(0.0, num_covars_excl_cost)))
            coeffs_cost = np.concatenate((np.tile(0.0, num_covar_excl_bene_ex_ante), coeffs_cost))
    ...
    def get_values(self, version, which):
        ...
        for para_obj in para_objs:
            ...
            if version == 'external':
                value = self._transform_to_external(para_obj, para_obj.get_attr('value'))
    ...
    def _transform_to_internal(self, para_obj, external_value):
        ...
        # Upper and lower bounds
            ...
            interval = upper_bound - lower_bound
            internal_value = lower_bound + interval/(1.0 + np.exp(-external_value)) 
